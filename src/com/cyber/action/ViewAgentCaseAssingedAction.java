 ...
 */
public class ViewAgentCaseAssingedAction extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			vcas = new CaseDelegate().viewAgentCaseAssinged((Integer) (session
					.getAttribute("userid")));
			if (!vcas.isEmpty()) {
				request
						.setAttribute(
								"status",
								UtilConstants._VIEW_AGENT_ASSIGNED_CASE_DETAILS_SUCCESS);
				request.setAttribute("assigniedcases", vcas);
				path = UtilConstants._VIEW_AGENT_ASSIGNED_CASES;
			} else {
				request.setAttribute("status",
						UtilConstants._VIEW_AGENT_ASSIGNED_CASE_DETAILS_FAIL);
				path = UtilConstants._VIEW_AGENT_ASSIGNED_CASES;
			}
		} catch (Exception e) {
